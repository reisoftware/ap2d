// TabCtrl_Left.cpp: implementation of the TabCtrl_Left class.
//
//////////////////////////////////////////////////////////////////////
...
#define TXT_FONT_DIALOG_INTERFACE  _TEXT("宋体\0")
#ifdef _DEBUG
...
IMPLEMENT_DYNCREATE(TabCtrl_Left, CWnd)
...
TabCtrl_Left::TabCtrl_Left()
MCT_INIT1("TabCtrl_Left")
{
	m_nStyle = TCS_DOWN;
	m_nActivePage = -1;
	m_nBorder = 1;
...
BEGIN_MESSAGE_MAP(TabCtrl_Left, CWnd)
	//{{AFX_MSG_MAP(TabCtrl_Left)
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_LBUTTONDOWN()
	ON_WM_SIZING()
...
/////////////////////////////////////////////////////////////////////////////
// TabCtrl_Left message handlers

BOOL TabCtrl_Left::Create(UINT wStyle, const CRect &rect, CWnd *pParentWnd, UINT nID)
{
...
BOOL TabCtrl_Left::OnEraseBkgnd(CDC* pDC) 
{
	if(m_bEraseBkgnd)
	{
		CRect rect;
...
TabCtrl_Left::CPageItem* TabCtrl_Left::AddPage(CWnd *pWnd, LPCTSTR sText, UINT IconID)
{
	ASSERT(pWnd);
...
BOOL TabCtrl_Left::AddPage(CRuntimeClass* pClass,UINT nIDTemplate, LPCTSTR sText, UINT IconID)
{
	...
	if(pDlg != NULL)
	{
		...
		{
			TabCtrl_Left::CPageItem *pItem = AddPage(pDlg, sText, IconID);
			if(pItem)
			{
...
void TabCtrl_Left::GetClientRect(LPRECT lpRect)
{
...
///////取得需要占用的宽度/////
UINT TabCtrl_Left::CPageItem::GetAreaWidth(CDC *pDC)
{
...
void TabCtrl_Left::CPageItem::Draw(CDC *pDC, UINT nStyle, BOOL bActive)
{
	CRect rect = m_rect;
	if(nStyle&TCS_DOWN)
	{
...
void TabCtrl_Left::SetActivePage(int nIndex)
{
...
void* TabCtrl_Left::GetPageItem(UINT nIndex)
{
	CPageItem *pItem = NULL;
...
void TabCtrl_Left::UpdateWindow()
{
 	AutoSize();
 	if(m_nActivePage < 0)
 		SetActivePage(0);
...
void TabCtrl_Left::OnLButtonDown(UINT nFlags, CPoint point) 
{
	UINT nItemIndex=0;
	POSITION pos;
	CPageItem *pItem;
...
void TabCtrl_Left::OnSizing(UINT fwSide, LPRECT pRect) 
{
...
void TabCtrl_Left::OnSize(UINT nType, int cx, int cy) 
{
...
void TabCtrl_Left::DrawFrame(CDC *pDC)
{
	CRect rect;
	GetClientRect(&rect);
	switch(m_nBorder)
...
void TabCtrl_Left::SetStyle(UINT style)
{
	m_nStyle = style;
}

UINT TabCtrl_Left::GetStyle()
{
	return m_nStyle;
}

void TabCtrl_Left::OnTimer(UINT nIDEvent) 
{
...
void TabCtrl_Left::SetStyleAnimate(BOOL bEnable)
{
	UINT style = GetStyle();
	if(bEnable)
		style |= TCS_ANIMATE;
...
void TabCtrl_Left::SetStyleDirection(int Direction)
{
	UINT style = GetStyle();
	switch(Direction)
	{
...
void TabCtrl_Left::SetStyleMonoSpace(BOOL bEnable)
{
	UINT style = GetStyle();
	if(bEnable)
		style |= TCS_MONOSPACE;
...
BOOL TabCtrl_Left::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
...
int TabCtrl_Left::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	strcpy(logFont.lfFaceName,TXT_FONT_DIALOG_INTERFACE);
	logFont.lfHeight = -12;
