...
Scene::~Scene()
{
////edit_ent_dlg_ 外部指针，不在此类管理器内存
//  	if(edit_ent_dlg_ != NULL)
//  		delete edit_ent_dlg_;
}
...
void Scene::move_scene(const Entity * e, double scale)
{
	cmd_list_.current()->move_scene(e,scale);
}
void Scene::move_scene(const std::vector<Entity*>& es, double scale)
{
	cmd_list_.current()->move_scene(es,scale);
}
...
void Scene::array_cad(const Entity_Array& ary)
{
  cmd_list_.current()->array_cad(ary);
}
...
{
	assert(cmd_list_.current() && "Scene::draw_scene(), cur cmd cann't be null");
...
{
	assert(window && "Scene::attach_window()");
...
void Scene::property_cad()
{
	cmd_list_.current()->property_cad();
}
...
void Scene::idle()
{
	cmd_list_.current()->idle_p();
}

void Scene::cmdin()
{
	cmd_list_.current()->cmdin();
}

void Scene::left_button(UINT nFlags, const Point& pt)
{
//Point pp = pt;
//world_to_screen_point(pp);
//cmd_list_.current()->mouse_move(nFlags, pp);
	cmd_list_.current()->left_button(nFlags, pt);
}

int Scene::screen_cx()const 
{
  RECT rc;
  painter_->get_client_rect(&rc);
  int cx = abs(rc.right-rc.left);
  return cx;
}
int Scene::screen_cy()const 
{
  RECT rc;
  painter_->get_client_rect(&rc);
  int cy = abs(rc.bottom-rc.top);
  return cy;
}

void Scene::screen_rc(int& cx, int& cy)const
{
  RECT rc;
  painter_->get_client_rect(&rc);
  cx = abs(rc.right-rc.left);
  cy = abs(rc.bottom-rc.top);
}

void Scene::screen_rc(RECT& rc)const
{
  painter_->get_client_rect(&rc);
}
