...
#ifndef TIAN_OBJECT_LN_DIM_PLATESIDE_H_
#define TIAN_OBJECT_LN_DIM_PLATESIDE_H_
...
//typedef void (*pf_ln_dim_text)(Ln_Dim_PlateSide& dim);
class OBJECT_API Ln_Dim_PlateSide : public Dimension
{
  MY_DECLARE_DYNCREATE(Ln_Dim_PlateSide)
public:
  virtual MY_CLASS myclass()const{return LINE_DIM;}
public:
  Ln_Dim_PlateSide();
  Ln_Dim_PlateSide(const Ln_Dim_PlateSide& rhs);
  Ln_Dim_PlateSide& operator=(const Ln_Dim_PlateSide& rhs);
  virtual ~Ln_Dim_PlateSide();
...
public:
  ...
  Point edm()const			                    {return edm_;}
	...
	void  direct(const Point& pt);
  Point direct()const			                    {return direct_;}
