...
namespace dlhml{
MY_IMPLEMENT_DYNCREATE(Ln_Dim_Direct,Dimension)
Ln_Dim_Direct::Ln_Dim_Direct()
MCT_INIT1("Ln_Dim_Direct")
{
...
Ln_Dim_Direct::Ln_Dim_Direct(const Ln_Dim_Direct& rhs)
:Dimension(rhs)
MCT_INIT2("Ln_Dim_Direct")
{
...
Ln_Dim_Direct& Ln_Dim_Direct::operator=(const Ln_Dim_Direct& rhs)
{
...
Float Ln_Dim_Direct::dim_line_radian() const
{
  return dim_line_radian_;
}
void Ln_Dim_Direct::dim_line_degree(Float a)
{
  //double rad = degree_2_radian(deg);
  dim_line_radian_ = degree_2_radian(a);
  show_change(true);
}
Float Ln_Dim_Direct::dim_line_degree() const
{
  return radian_2_degree(dim_line_radian_);
}
    

bool Ln_Dim_Direct::visit(Visitor& v)
{
  show_change(true);
	return v.Visit_Ln_Dim_Direct(*this);
}
//void Ln_Dim_Direct::text3_normal(const Normal& nor)
//{
//	text3_rotation_ = nor.angle_x_axis_two_pi();
//  show_change(true);
//}
void Ln_Dim_Direct::loadme(File& in)
{
...
void Ln_Dim_Direct::saveme(const File& out) const
{
	save_me(out);
	out << dim_mode_
      << dim_rota_
...
void Ln_Dim_Direct::save_luame(std::ofstream &out,int tab_num)
{
...
void Ln_Dim_Direct::open_luame(lua_State *l)  
{
	TRACE_OUT("Ln_Dim_Direct::open_luame() start\n");
	...
	txt_change(true);
	TRACE_OUT("Ln_Dim_Direct::open_luame() end\n");

}
void Ln_Dim_Direct::get_strme(char *out)
{
	rei_quit();
...
void Ln_Dim_Direct::set_from_strme(char *in)
{
	rei_init(in);
	set_from_str_ent();
	set_from_str_dim();
...
void Ln_Dim_Direct::get_str_sub(char *name)
{
...
void Ln_Dim_Direct::set_from_str_sub(char *name)
{
	char s[MAX_REDIS_STR];
...
void Ln_Dim_Direct::edit_propertyme(const Edit_Entity& edit)
{
//	edit.edit_ln_dim(*this);
  show_change(true);
}

//void Ln_Dim_Direct::drawme(Draw_Entity& out) const
//{
//	out.draw_ln_dim(*this);
//}
//void Ln_Dim_Direct::xor_drawme(Draw_Entity& out) const
//{
//	out.xor_draw_ln_dim(*this);
//}

errorinfo Ln_Dim_Direct::postme2db(Database& db,Entity_ID& id)
{
  show_change(true);
	return db.entity_push_back(this,id);
}

void Ln_Dim_Direct::transformbyme(const Matrix& mat)
{
// 	REITRACE;
...
void Ln_Dim_Direct::drag_startme(const Matrix& mat)
{
...
void Ln_Dim_Direct::drag_endme(const Matrix& mat)
{
	text2_position_.transformby(mat);
	text3_position_.transformby(mat);
	edm_.transformby(mat);
...
Entity* Ln_Dim_Direct::cloneme() const
{
	return new Ln_Dim_Direct(*this);
}

void Ln_Dim_Direct::closeme()
{
	delete this;
}

//TEXT2 SET
void Ln_Dim_Direct::dim_text2(std::string str)			          
{
  (dim_text2_ = str);
  txt2_change(true);
}
void Ln_Dim_Direct::text2_rotation(Float f)		          
{
  text2_rotation_=f;
  txt2_change(true);
}
void Ln_Dim_Direct::text2_position(const Point& pos)	  
{
  text2_position_=pos;
  txt2_change(true);
}
void Ln_Dim_Direct::text2_normal(const Normal& nor)
{
  text2_rotation_ = nor.angle_x_axis_two_pi();
  txt2_change(true);
}
void Ln_Dim_Direct::align_point2(Text::AlignPt align)	  
{
  align_point2_=align;
  txt2_change(true);
}
...
//DATA
void Ln_Dim_Direct::setdata(const Point& opt,
                     const Point& spt,
                     const Point& ept,
                     const Point& stdim,
                     const Point& eddim)
...
void Ln_Dim_Direct::opt(const Point& pt)			
{
  opt_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_Direct::spt(const Point& pt)			
{
  spt_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_Direct::ept(const Point& pt)				
{
  ept_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_Direct::odm(const Point& pt)		
{
  odm_.set2d(pt);
  txt2_change(true);
}

void Ln_Dim_Direct::edm(const Point& pt)		
{
  edm_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_Direct::ext_line_radian(Float a)
{
  dim_line_radian_ = a-pi/2;
  txt2_change(true);
}
void Ln_Dim_Direct::dim_line_radian(Float a)
{
  dim_line_radian_ = a;
  txt2_change(true);
}
...
//MODE
void Ln_Dim_Direct::mode(DIM_MODE m)
{
  txt2_change(true);
  dim_mode_=m;
}
void Ln_Dim_Direct::rota(DIM_ROTA m)
{
  txt2_change(true);
  dim_rota_=m;
}
...
void Ln_Dim_Direct::align_dim_lines(const Point& pos)
{
  txt2_change(true);
...
void Ln_Dim_Direct::align_base(const Point& pos)
{
  txt2_change(true);
  Entity_Calculate calc;
  Line oexln;
...
void Ln_Dim_Direct::offset_dm(const Normal off_nor, Float dis, int flags)
{
  show_change(true);
  Normal nor = off_nor;
  ext_line_normal(nor);
...
//TEXT RECORD
void Ln_Dim_Direct::txt2_change(bool b)const
{
  Ln_Dim_Direct * the = const_cast<Ln_Dim_Direct*>(this);
  the->txt2_change_=b;

  txt_change(true);
  show_change(true);
}
void Ln_Dim_Direct::txt2_record(const Text& txt)const
{
  Ln_Dim_Direct * the = const_cast<Ln_Dim_Direct*>(this);
  the->txt2_record_=txt;
  txt2_change(false);
}
bool Ln_Dim_Direct::get_text2_show(Text& text) const
{
  if(txt2_change_){
...
bool Ln_Dim_Direct::get_text3_show(Text& text) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
bool Ln_Dim_Direct::get_text2_frame_show(Text& text, std::vector<Entity*>& es)const
{
  get_text2_show(text);
  dim_style_val_.get_text_frame(text,es);
  return true;
}
bool Ln_Dim_Direct::pt_in_range(const Point& pt) const
{
...
bool Ln_Dim_Direct::get_o_lines(Line& oexln,Line& odmln) const
{
//	if(!start_show_)
//		return false;
  //bet
...
bool Ln_Dim_Direct::get_s_lines(Line& sexln,Line& eexln,Line& sdmln) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
bool Ln_Dim_Direct::get_e_lines(Line& extendline,Line& dimline) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
void Ln_Dim_Direct::get_line(std::vector<Entity*>& es)const
{
...
void Ln_Dim_Direct::get_show(std::vector<Entity*>& es) const
{
  switch(dim_mode_)
  {
  case PILEUP_DIM:
...
void Ln_Dim_Direct::get_pileup_show(std::vector<Entity*>& es) const
{
  //0
  if(show0_){
    //line
...
void Ln_Dim_Direct::get_closed_show(std::vector<Entity*>& es) const
{
  //line
  Line * sex = Line::create_me();
  Line * eex = Line::create_me();
...
bool Ln_Dim_Direct::get_dim_lines_show(std::vector<Line*>& lines) const
{
  int i=0;
...
//void Ln_Dim_Direct::push_o_arrow_line_show(std::vector<Entity*>& es)const
//{
...
//  }
//}
int Ln_Dim_Direct::get_o_arrow_line_show(std::vector<Line*>& lines) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//void Ln_Dim_Direct::push_e_arrow_line_show(std::vector<Entity*>& es)const
//{
//  std::vector<Line*> oln_arraw;
//  oln_arraw.resize(ARROW_LINE_COUNT);
//  int i=0;
...
//  }
//}
int Ln_Dim_Direct::get_e_arrow_line_show(std::vector<Line*>& lines) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//  获得0点箭头线
//void Ln_Dim_Direct::push_o_arrow_circle_show(std::vector<Entity*>& es)const
//{
...
//  }
//}
int Ln_Dim_Direct::get_o_arrow_circle_show(std::vector<Circle*>& circles) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//void Ln_Dim_Direct::push_e_arrow_circle_show(std::vector<Entity*>& es)const
//{
//  std::vector<Circle*> oln_arraw;
//  oln_arraw.resize(ARROW_LINE_COUNT);
//  int i=0;
...
//  }
//}
int Ln_Dim_Direct::get_e_arrow_circle_show(std::vector<Circle*>& circles) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//void Ln_Dim_Direct::push_s_arrow_line_show(std::vector<Entity*>& es)const
//{
//  Line sex, eex, edm;
//  get_s_lines(sex, eex, edm);
//
...
//  }
//}
int Ln_Dim_Direct::get_s_arrow_line_show(const Line& dim,std::vector<Line*>& lines) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//void Ln_Dim_Direct::push_s_arrow_circle_show(std::vector<Entity*>& es)const
//{
//  Line sex, eex, edm;
//  get_s_lines(sex, eex, edm);
//
...
//  }
//}
int Ln_Dim_Direct::get_s_arrow_circle_show(const Line& dim,std::vector<Circle*>& circles) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
bool Ln_Dim_Direct::intersectme(const Rect& rect) const
{
	Entity_Calculate calc;
...
bool Ln_Dim_Direct::envelopme(const Rect& rect) const
{
	Rect rc,rc2,rc3;
	Text text;
	get_text_show(text);
...
Rect Ln_Dim_Direct::calc_box2d() const
{
	Rect box;
...
//bool Ln_Dim_Direct::snap_intersect_pointme(Point& pt,const Entity& ent,const Rect& rect) const
//{
////NOOK SNAP 2009.07.02
//  return false;
////	Entity_Snap_Implement<Ln_Dim_Direct> imp(*this);
////	return ent.snap_intersect_point_imp(pt,imp,rect);
//}
bool Ln_Dim_Direct::snap_intersect_pointme(Point& pt,const Entity_Snap& snap,const Rect& rect) const
{
	return snap.intersect(pt,rect,*this);
}

//  //template<class T> 
//void Ln_Dim_Direct::get_entitits_show(std::vector<Entity*>& es,
...
void Ln_Dim_Direct::setctrl(const std::vector<Point>& pts)
{
...
void Ln_Dim_Direct::getctrl(std::vector<Point>& pts)const
{
  pts.push_back(opt_);
  pts.push_back(ept_);
  pts.push_back(edm_);
}
int Ln_Dim_Direct::pt_count()const
{
  return 3;
}

void Ln_Dim_Direct::ext_line_normal(Normal& nor)
{
  show_change(true);
  Line oexln,odmln;
  get_o_lines(oexln, odmln);
...
//void Ln_Dim_Direct::offset_dm(const Normal off_nor, int flags)
//{
//  Normal nor = off_nor;
//  ext_line_normal(nor);
//
...
Float Ln_Dim_Direct::ext_line_radian()const
{
  return dim_line_radian_ + dlhml::pi/2;
}


// Rect Ln_Dim_Direct::offset_box()    //  干涉后参与偏移部分的外框
// void offset_pp(); //  偏移位置++
// int  offset_floor();  //  起始楼层
// int  span_floor();    //  占用层数
// int  floor_count();   //  楼层数

Float Ln_Dim_Direct::get_span_oe() const
{
  Line oexln, eexln, odmln, edmln;
  get_o_lines(oexln, odmln);
  get_e_lines(eexln, edmln);
...
Float Ln_Dim_Direct::get_span_se() const
{
