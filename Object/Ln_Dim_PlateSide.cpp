...
namespace dlhml{
MY_IMPLEMENT_DYNCREATE(Ln_Dim_PlateSide,Dimension)
Ln_Dim_PlateSide::Ln_Dim_PlateSide()
MCT_INIT1("Ln_Dim_PlateSide")
{
...
Ln_Dim_PlateSide::Ln_Dim_PlateSide(const Ln_Dim_PlateSide& rhs)
:Dimension(rhs)
MCT_INIT2("Ln_Dim_PlateSide")
{
...
Ln_Dim_PlateSide& Ln_Dim_PlateSide::operator=(const Ln_Dim_PlateSide& rhs)
{
...
Float Ln_Dim_PlateSide::dim_line_radian() const
{
  return dim_line_radian_;
}
void Ln_Dim_PlateSide::dim_line_degree(Float a)
{
  //double rad = degree_2_radian(deg);
  dim_line_radian_ = degree_2_radian(a);
  show_change(true);
}
Float Ln_Dim_PlateSide::dim_line_degree() const
{
  return radian_2_degree(dim_line_radian_);
}
    

bool Ln_Dim_PlateSide::visit(Visitor& v)
{
  show_change(true);
	return v.Visit_Ln_Dim_PlateSide(*this);
}
//void Ln_Dim_PlateSide::text3_normal(const Normal& nor)
//{
//	text3_rotation_ = nor.angle_x_axis_two_pi();
//  show_change(true);
//}
void Ln_Dim_PlateSide::loadme(File& in)
{
...
void Ln_Dim_PlateSide::saveme(const File& out) const
{
	save_me(out);
	out << dim_mode_
      << dim_rota_
...
void Ln_Dim_PlateSide::save_luame(std::ofstream &out,int tab_num)
{
	...
	direct_.save_lua(out,"direct_",tab_num);
}
...
void Ln_Dim_PlateSide::open_luame(lua_State *l)  
{
	TRACE_OUT("Ln_Dim_PlateSide::open_luame() start\n");
	...
	direct_.open_lua(l,"direct_");
	txt_change(true);
	TRACE_OUT("Ln_Dim_PlateSide::open_luame() end\n");

}
void Ln_Dim_PlateSide::get_strme(char *out)
{
	rei_quit();
	...
	direct_.get_str_sub("direct_");

	rei_get_encode(out);
}
void Ln_Dim_PlateSide::set_from_strme(char *in)
{
	rei_init(in);
	set_from_str_ent();
	set_from_str_dim();
	...
	direct_.set_from_str_sub("direct_");
	rei_quit();
}
void Ln_Dim_PlateSide::get_str_sub(char *name)
{
	...
	odm_.get_str_sub("odm_");
	sprintf(s,"%s.direct_",name);
	direct_.get_str_sub("direct_");
}
void Ln_Dim_PlateSide::set_from_str_sub(char *name)
{
	char s[MAX_REDIS_STR];
...
void Ln_Dim_PlateSide::edit_propertyme(const Edit_Entity& edit)
{
//	edit.edit_ln_dim(*this);
  show_change(true);
}

//void Ln_Dim_PlateSide::drawme(Draw_Entity& out) const
//{
//	out.draw_ln_dim(*this);
//}
//void Ln_Dim_PlateSide::xor_drawme(Draw_Entity& out) const
//{
//	out.xor_draw_ln_dim(*this);
//}

errorinfo Ln_Dim_PlateSide::postme2db(Database& db,Entity_ID& id)
{
  show_change(true);
	return db.entity_push_back(this,id);
}

void Ln_Dim_PlateSide::transformbyme(const Matrix& mat)
{
// 	REITRACE;
...
void Ln_Dim_PlateSide::drag_startme(const Matrix& mat)
{
...
void Ln_Dim_PlateSide::drag_endme(const Matrix& mat)
{
	text2_position_.transformby(mat);
	text3_position_.transformby(mat);
	edm_.transformby(mat);
...
Entity* Ln_Dim_PlateSide::cloneme() const
{
	return new Ln_Dim_PlateSide(*this);
}

void Ln_Dim_PlateSide::closeme()
{
	delete this;
}

//TEXT2 SET
void Ln_Dim_PlateSide::dim_text2(std::string str)			          
{
  (dim_text2_ = str);
  txt2_change(true);
}
void Ln_Dim_PlateSide::text2_rotation(Float f)		          
{
  text2_rotation_=f;
  txt2_change(true);
}
void Ln_Dim_PlateSide::text2_position(const Point& pos)	  
{
  text2_position_=pos;
  txt2_change(true);
}
void Ln_Dim_PlateSide::text2_normal(const Normal& nor)
{
  text2_rotation_ = nor.angle_x_axis_two_pi();
  txt2_change(true);
}
void Ln_Dim_PlateSide::align_point2(Text::AlignPt align)	  
{
  align_point2_=align;
  txt2_change(true);
}
...
//DATA
void Ln_Dim_PlateSide::setdata(const Point& opt,
                     const Point& spt,
                     const Point& ept,
                     const Point& stdim,
                     const Point& eddim,const Point& direct)
{
  opt_.set2d(opt);
  spt_.set2d(spt);
  ept_.set2d(ept);
  odm_.set2d(stdim);
  edm_.set2d(eddim);
  direct_.set2d(direct);
  txt2_change(true);
}
void Ln_Dim_PlateSide::opt(const Point& pt)			
{
  opt_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_PlateSide::spt(const Point& pt)			
{
  spt_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_PlateSide::ept(const Point& pt)				
{
  ept_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_PlateSide::odm(const Point& pt)		
{
  odm_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_PlateSide::direct(const Point& pt)		
{
  direct_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_PlateSide::edm(const Point& pt)		
{
  edm_.set2d(pt);
  txt2_change(true);
}
void Ln_Dim_PlateSide::ext_line_radian(Float a)
{
  dim_line_radian_ = a-pi/2;
  txt2_change(true);
}
void Ln_Dim_PlateSide::dim_line_radian(Float a)
{
  dim_line_radian_ = a;
  txt2_change(true);
}
...
//MODE
void Ln_Dim_PlateSide::mode(DIM_MODE m)
{
  txt2_change(true);
  dim_mode_=m;
}
void Ln_Dim_PlateSide::rota(DIM_ROTA m)
{
  txt2_change(true);
  dim_rota_=m;
}
...
void Ln_Dim_PlateSide::align_dim_lines(const Point& pos)
{
  txt2_change(true);
...
void Ln_Dim_PlateSide::align_base(const Point& pos)
{
  txt2_change(true);
  Entity_Calculate calc;
  Line oexln;
...
void Ln_Dim_PlateSide::offset_dm(const Normal off_nor, Float dis, int flags)
{
  show_change(true);
  Normal nor = off_nor;
  ext_line_normal(nor);
...
//TEXT RECORD
void Ln_Dim_PlateSide::txt2_change(bool b)const
{
  Ln_Dim_PlateSide * the = const_cast<Ln_Dim_PlateSide*>(this);
  the->txt2_change_=b;

  txt_change(true);
  show_change(true);
}
void Ln_Dim_PlateSide::txt2_record(const Text& txt)const
{
  Ln_Dim_PlateSide * the = const_cast<Ln_Dim_PlateSide*>(this);
  the->txt2_record_=txt;
  txt2_change(false);
}
bool Ln_Dim_PlateSide::get_text2_show(Text& text) const
{
  if(txt2_change_){
...
bool Ln_Dim_PlateSide::get_text3_show(Text& text) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
bool Ln_Dim_PlateSide::get_text2_frame_show(Text& text, std::vector<Entity*>& es)const
{
  get_text2_show(text);
  dim_style_val_.get_text_frame(text,es);
  return true;
}
bool Ln_Dim_PlateSide::pt_in_range(const Point& pt) const
{
...
bool Ln_Dim_PlateSide::get_o_lines(Line& oexln,Line& odmln) const
{
//	if(!start_show_)
//		return false;
  //bet
...
bool Ln_Dim_PlateSide::get_s_lines(Line& sexln,Line& eexln,Line& sdmln) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
bool Ln_Dim_PlateSide::get_e_lines(Line& extendline,Line& dimline) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
void Ln_Dim_PlateSide::get_line(std::vector<Entity*>& es)const
{
...
void Ln_Dim_PlateSide::get_show(std::vector<Entity*>& es) const
{
  switch(dim_mode_)
  {
  case PILEUP_DIM:
...
void Ln_Dim_PlateSide::get_pileup_show(std::vector<Entity*>& es) const
{
  //0
  if(show0_){
    //line
...
void Ln_Dim_PlateSide::get_closed_show(std::vector<Entity*>& es) const
{
  //line
  Line * sex = Line::create_me();
  Line * eex = Line::create_me();
...
bool Ln_Dim_PlateSide::get_dim_lines_show(std::vector<Line*>& lines) const
{
  int i=0;
...
//void Ln_Dim_PlateSide::push_o_arrow_line_show(std::vector<Entity*>& es)const
//{
...
//  }
//}
int Ln_Dim_PlateSide::get_o_arrow_line_show(std::vector<Line*>& lines) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//void Ln_Dim_PlateSide::push_e_arrow_line_show(std::vector<Entity*>& es)const
//{
//  std::vector<Line*> oln_arraw;
//  oln_arraw.resize(ARROW_LINE_COUNT);
//  int i=0;
...
//  }
//}
int Ln_Dim_PlateSide::get_e_arrow_line_show(std::vector<Line*>& lines) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//  获得0点箭头线
//void Ln_Dim_PlateSide::push_o_arrow_circle_show(std::vector<Entity*>& es)const
//{
...
//  }
//}
int Ln_Dim_PlateSide::get_o_arrow_circle_show(std::vector<Circle*>& circles) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//void Ln_Dim_PlateSide::push_e_arrow_circle_show(std::vector<Entity*>& es)const
//{
//  std::vector<Circle*> oln_arraw;
//  oln_arraw.resize(ARROW_LINE_COUNT);
//  int i=0;
...
//  }
//}
int Ln_Dim_PlateSide::get_e_arrow_circle_show(std::vector<Circle*>& circles) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//void Ln_Dim_PlateSide::push_s_arrow_line_show(std::vector<Entity*>& es)const
//{
//  Line sex, eex, edm;
//  get_s_lines(sex, eex, edm);
//
...
//  }
//}
int Ln_Dim_PlateSide::get_s_arrow_line_show(const Line& dim,std::vector<Line*>& lines) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
//void Ln_Dim_PlateSide::push_s_arrow_circle_show(std::vector<Entity*>& es)const
//{
//  Line sex, eex, edm;
//  get_s_lines(sex, eex, edm);
//
...
//  }
//}
int Ln_Dim_PlateSide::get_s_arrow_circle_show(const Line& dim,std::vector<Circle*>& circles) const
{
  //bet
  //2009.06.18
	const Dimension_Style* ds=&dim_style_val_;
...
bool Ln_Dim_PlateSide::intersectme(const Rect& rect) const
{
	Entity_Calculate calc;
...
bool Ln_Dim_PlateSide::envelopme(const Rect& rect) const
{
	Rect rc,rc2,rc3;
	Text text;
	get_text_show(text);
...
Rect Ln_Dim_PlateSide::calc_box2d() const
{
	Rect box;
...
//bool Ln_Dim_PlateSide::snap_intersect_pointme(Point& pt,const Entity& ent,const Rect& rect) const
//{
////NOOK SNAP 2009.07.02
//  return false;
////	Entity_Snap_Implement<Ln_Dim_PlateSide> imp(*this);
////	return ent.snap_intersect_point_imp(pt,imp,rect);
//}
bool Ln_Dim_PlateSide::snap_intersect_pointme(Point& pt,const Entity_Snap& snap,const Rect& rect) const
{
	return snap.intersect(pt,rect,*this);
}

//  //template<class T> 
//void Ln_Dim_PlateSide::get_entitits_show(std::vector<Entity*>& es,
//  int (Ln_Dim_PlateSide::*show_ptr)(std::vector<Line*>))const
//{
//  std::vector<Line*> oln_arraw;
//  oln_arraw.resize(ARROW_LINE_COUNT);
//  int i=0;
...
void Ln_Dim_PlateSide::setctrl(const std::vector<Point>& pts)
{
...
void Ln_Dim_PlateSide::getctrl(std::vector<Point>& pts)const
{
  pts.push_back(opt_);
  pts.push_back(ept_);
  pts.push_back(edm_);
}
int Ln_Dim_PlateSide::pt_count()const
{
  return 3;
}

void Ln_Dim_PlateSide::ext_line_normal(Normal& nor)
{
  show_change(true);
  Line oexln,odmln;
  get_o_lines(oexln, odmln);
...
//void Ln_Dim_PlateSide::offset_dm(const Normal off_nor, int flags)
//{
//  Normal nor = off_nor;
//  ext_line_normal(nor);
//
...
Float Ln_Dim_PlateSide::ext_line_radian()const
{
  return dim_line_radian_ + dlhml::pi/2;
}


// Rect Ln_Dim_PlateSide::offset_box()    //  干涉后参与偏移部分的外框
// void offset_pp(); //  偏移位置++
// int  offset_floor();  //  起始楼层
// int  span_floor();    //  占用层数
// int  floor_count();   //  楼层数

Float Ln_Dim_PlateSide::get_span_oe() const
{
  Line oexln, eexln, odmln, edmln;
  get_o_lines(oexln, odmln);
  get_e_lines(eexln, edmln);
...
Float Ln_Dim_PlateSide::get_span_se() const
{
