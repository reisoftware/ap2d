#ifndef _DIM_ANGLE_LEAN_ARROW_H_
#define _DIM_ANGLE_LEAN_ARROW_H_
...
class OBJECT_API Dim_Angle_Lean : public Dimension
{
	MY_DECLARE_DYNCREATE(Dim_Angle_Lean)

public:
	Dim_Angle_Lean();
  Dim_Angle_Lean(const Line & ln);
	Dim_Angle_Lean(const Point& st,const Point& ed);
  Dim_Angle_Lean(const Dim_Angle_Lean& rhs);
  Dim_Angle_Lean& operator=(const Dim_Angle_Lean& rhs);
	virtual ~Dim_Angle_Lean();
