...
#ifndef TIAN_OBJECT_LN_DIM_H_
#define TIAN_OBJECT_LN_DIM_H_
...
typedef void (*pf_ln_dim_text)(Ln_Dim& dim);
class OBJECT_API Ln_Dim : public Dimension
{
  MY_DECLARE_DYNCREATE(Ln_Dim)
public:
  virtual MY_CLASS myclass()const{return LINE_DIM;}
public:
  Ln_Dim();
  Ln_Dim(const Ln_Dim& rhs);
  Ln_Dim& operator=(const Ln_Dim& rhs);
  virtual ~Ln_Dim();
