// Lead_Dimension.cpp: implementation of the Lead_Dimension class.
//
//////////////////////////////////////////////////////////////////////
...
namespace dlhml{
MY_IMPLEMENT_DYNCREATE(Lead_Dimension,Dimension)
Lead_Dimension::Lead_Dimension()
MCT_INIT1("Lead_Dimension")
{
...
Lead_Dimension::Lead_Dimension(const Lead_Dimension& rhs)
:Dimension(rhs)
MCT_INIT2("Lead_Dimension")
{
	//dim_content_ = NULL;
	dim_sign_ = NULL;
	start_ = rhs.start_;
...
Lead_Dimension& Lead_Dimension::operator=(const Lead_Dimension& rhs)
{
...
void Lead_Dimension::setdata(const Point& st,const Point& ed)
{
  show_change(true);
	start_ = st;
  text_position(ed);
...
void  Lead_Dimension::start(const Point& st)
{
  show_change(true);
  start_=st;
}
Point Lead_Dimension::start()const
{
  return start_;
}


void Lead_Dimension::end(const Point& ed)				
{
  txt_change(true);
  show_change(true);
  text_position(ed);
...
Point Lead_Dimension::end()const
{
  return text_position();
}


bool Lead_Dimension::visit(Visitor& v)
{
  show_change(true);
	return v.Visit_Lead_Dimension(*this);
}
void Lead_Dimension::loadme(File& in)
{
	load_me(in);
	start_.load(in);
	//end_.load(in);
...
void Lead_Dimension::saveme(const File& out) const
{
	save_me(out);
	start_.save(out);
	//end_.save(out);
...
void Lead_Dimension::save_luame(std::ofstream &out,int tab_num)
{
...
void Lead_Dimension::open_luame(lua_State *l)  
{
	TRACE_OUT("Lead_Dimension::open_luame() start\n");
	...
	txt_change(true);
	TRACE_OUT("Lead_Dimension::open_luame() end\n");

}
void Lead_Dimension::get_strme(char *out)
{
	rei_quit();
...
void Lead_Dimension::set_from_strme(char *in)
{
	rei_init(in);
	set_from_str_ent();
	set_from_str_dim();
...
void Lead_Dimension::get_str_sub(char *name)
{
...
void Lead_Dimension::set_from_str_sub(char *name)
{
	char s[MAX_REDIS_STR];
...
//void Lead_Dimension::dim_content(LPCTSTR dim_content)
//{
// txt_change(true);
// string_copy(dim_content_,dim_content);
//}
//LPCTSTR Lead_Dimension::dim_content() const
//{
//	return dim_content_;
//}
void Lead_Dimension::dim_sign(LPCTSTR dim_sign)
{
 txt_change(true);
 string_copy(dim_sign_,dim_sign);
}
LPCTSTR Lead_Dimension::dim_sign() const
{
	return dim_sign_;
}
void Lead_Dimension::edit_propertyme(const Edit_Entity& edit)
{
  show_change(true);
//	edit.edit_lead_dim(*this);
}

//void Lead_Dimension::drawme(Draw_Entity& out) const
//{
//	out.draw_lead_dim(*this);
//}
//
//void Lead_Dimension::xor_drawme(Draw_Entity& out) const
//{
//	out.xor_draw_lead_dim(*this);
//}

errorinfo Lead_Dimension::postme2db(Database& db,Entity_ID& id)
{
  show_change(true);
	return db.entity_push_back(this,id);
}

void Lead_Dimension::transformbyme(const Matrix& mat)
{
//	REITRACE;
  show_change(true);
//	REITRACE;
...
void Lead_Dimension::drag_startme(const Matrix& mat)
{
  show_change(true);
	start_.transformby(mat);
	start_pt_rect_.transformby(mat);
}
void Lead_Dimension::drag_endme(const Matrix& mat)
{
  show_change(true);
	text_position_.transformby(mat);
...
Entity* Lead_Dimension::cloneme() const
{
	return new Lead_Dimension(*this);
}
bool Lead_Dimension::start_in_rect() const
{
	Rect zero;
...
void Lead_Dimension::closeme()
{
	delete this;
}
bool Lead_Dimension::get_line(Line& line) const
{
  //Point ed;
  Text txt;
  get_text_show(txt);
...
void Lead_Dimension::get_line(std::vector<Entity*>& es)const
{
  //LINE
...
void Lead_Dimension::get_show(std::vector<Entity*>& es)const
{
	////不显示牵引线
	if(lead_){
		get_line(es);
...
void Lead_Dimension::get_line_show(Line& ln)const
{
	get_line(ln);
...
//bool Lead_Dimension::get_line_show(std::vector<Line*>& lines) const
//{
//	get_line(*lines[0]);
...
int Lead_Dimension::get_start_arrow_line_show(std::vector<Line*>& lines) const
{
  Line start;
  get_line(start);
...
int Lead_Dimension::get_start_arrow_circle_show(std::vector<Circle*>& circles) const
{
  Line start;
  get_line(start);
  const Dimension_Style* ds=&dim_style_val_;
...
bool Lead_Dimension::intersectme(const Rect& rect) const
{
	Line line;
	get_line(line);
...
bool Lead_Dimension::envelopme(const Rect& rect) const
{
	Text text;
	get_text_show(text);
...
//Rect Lead_Dimension::calc_box2d() const
//{
//	Rect box2d_;
//	Line line;
//	get_line(line);
...
//	return box2d_;
//}
Rect Lead_Dimension::get_txt_rect()
{
	Rect rect;
	Text text;
	get_text_show(text);
...
bool Lead_Dimension::snap_endpointme(Point& pt,const Rect& rect) const
{
	Line line;
	get_line(line);
	return line.snap_endpoint(pt,rect);
}
bool Lead_Dimension::snap_middle_pointme(Point& pt,const Rect& rect) const
{
	Line line;
	get_line(line);
	return line.snap_middle_point(pt,rect);
}
bool Lead_Dimension::snap_nearest_pointme(Point& pt,const Rect& rect) const
{
	Line line;
	get_line(line);
	return line.snap_nearest_point(pt,rect);
}
//bool Lead_Dimension::snap_intersect_pointme(Point& pt,const Entity& ent,const Rect& rect) const
//{
////NOOK SNAP 2009.07.02
//////Entity 统一实现
//  return false;
////	Entity_Snap_Implement<Lead_Dimension> imp(*this);
////	return ent.snap_intersect_point_imp(pt,imp,rect);
//}
bool Lead_Dimension::snap_intersect_pointme(Point& pt,const Entity_Snap& snap,const Rect& rect) const
{
	return snap.intersect(pt,rect,*this);
}
Point Lead_Dimension::get_range_pt() 
{
...
//		//end_ = pts[1];
//09.6.22
void Lead_Dimension::setctrl(const std::vector<Point>& pts)
{
  show_change(true);
	assert(pts.size() >= 1);
	start_ = pts[0];
...
void Lead_Dimension::getctrl(std::vector<Point>& pts) const
{
	if(lead_){
...
int  Lead_Dimension::pt_count()const
{
  return lead_ ? 2 : 1;
}
void Lead_Dimension::setdrag(const std::vector<Point>& pts, int index)
{
...
void Lead_Dimension::getdrag(std::vector<Point>& pts) const
{
	pts.push_back(text_position());
	if(lead_){
		pts.push_back(start_);
