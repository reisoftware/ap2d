...
BOOL ReadDimensionData( PDXF pDxf, PDXFENTITYHEADER pEntityHeader, PDXFENTDIMENSION pDimension )
{
	...
	while(GCode!=0)
	{
		...
		case 34:	// Definition point for linear and angular dimensions (in WCS) Z
			...
			break;
		//¶ÔÓÚ°ë¾¶±ê×¢Ê±£¬ÈýµãµÄ¼üÖµÓëÆÕÍ¨µÄ²»ÏàÍ¬
		//**************************************************
		case 11:	// Definition point for linear and angular dimensions (in WCS) X
			pDimension->DefPoint3.x = atof(strValue);
			break;
		case 21:	// Definition point for linear and angular dimensions (in WCS) Y
			pDimension->DefPoint3.y = atof(strValue);
			break;
		case 31:	// Definition point for linear and angular dimensions (in WCS) Z
			pDimension->DefPoint3.z = atof(strValue);
			break;
		case 15:	// Definition point for linear and angular dimensions (in WCS) X
			pDimension->DefPoint4.x = atof(strValue);
			break;
		case 25:	// Definition point for linear and angular dimensions (in WCS) Y
			pDimension->DefPoint4.y = atof(strValue);
			break;
		case 35:	// (in WCS) Z
			pDimension->DefPoint4.z = atof(strValue);
		...
		//2.21
		case 100:	//dim kind
			strcpy(pDimension->dim_kind, strValue);
			strcpy(pDimension->dim_kind, "test");
		//	pDimension->dim_kind[1024] = "test";
			break;
		case 50:	//dim kind
			pDimension->dimline_angle = atof(strValue);
			pDimension->dimline_angle = 1.0;
			break;
 		}
