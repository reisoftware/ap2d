...
#include "..\CadIO\DrawingDataChange.h"
BOOL drw_get_ents(HDRAWING hDrawing,dlhml::Database& database)
{
	...
	{
		do{
			deal_ents(pDrawing,hDrawing,&EntityHeader,&EntityData,database);
		}
		while(drwFindEntity_Direct(pDrawing, NULL, &EntityHeader, &EntityData, FIND_NEXT)>0);
	}
...
BOOL deal_ents(PDRAWING pDrawing,HDRAWING hDrawing,PENTITYHEADER pEntityHeader, LPVOID pEntityData,dlhml::Database & database)
{
	...
	CDrawingDataChange::GetLtype(ltype.Name,entProp.ch_ltype);
	//ÑÕÉ«
	int iIndexColor = 0;
	iIndexColor = get_color(hDrawing,pEntityHeader,pEntityHeader->Color);
	entProp.color = CDrawingDataChange::GetColor(iIndexColor);

	switch( pEntityHeader->EntityType )
	{
	case ENT_LINE:
		result = get_line(entProp,pEntityHeader,(PENTLINE)pEntityData,database);
		break;
	case ENT_POINT:
		break;
	case ENT_CIRCLE:
		result = get_circle(entProp,pEntityHeader,(PENTCIRCLE)pEntityData,database);
		break;
	case ENT_TEXT:
		result = get_text(entProp,pEntityHeader,(PENTTEXT)pEntityData,database);
		break;
	case ENT_ARC:
		result = get_arc(entProp,pEntityHeader,(PENTARC)pEntityData,database);
		break;
//	case ENT_ELLIPSE:
//		break;
	case ENT_SOLID:
...
/*
	...
	pNewLine->createme();
	Point ptStart,ptEnd;
	ptStart.setdata(pLine->Point0.x,pLine->Point0.y);
	ptEnd.setdata(pLine->Point1.x,pLine->Point1.y);
	pNewLine->color(entProp.color);
	pNewLine->layername(entProp.ch_layer);
	pNewLine->linestyle(entProp.ch_ltype);
	pNewLine->setdata(ptStart,ptEnd);
	Entity_ID id;
	pNewLine->post2db(database,id);
*/
	return TRUE;
...
//	LAYER layer;//
//	get_layer(hDrawing,pEntityHeader,layer);
//	ltype.Objhandle = layer.LineTypeObjhandle;	
	DWORD dwJudge = if_by_layer(ZB_LTYPE,pEntityHeader->LTypeObjhandle);
	if(ZB_BYLAYER & dwJudge){
		LAYER layer;
		get_layer(hDrawing,pEntityHeader,layer);
		ltype.Objhandle = layer.LineTypeObjhandle;	
	}
...
{
	style.Objhandle = pEntityHeader->LayerObjhandle;
	if(drwFindTableType(hDrawing, TAB_STYLE, FIND_BYHANDLE, &style)>0)
		return TRUE;
	else
		return FALSE;
...
{
	dimStyle.Objhandle = pEntityHeader->LayerObjhandle;
...
{
	if(dwKind & ZB_COLOR){
		//¶ÔÓÚcolor £¬0ÊÇËæ¿é£¬256ÊÇËæ²ã
		if(iID == 0)
			return ZB_BYBLOCK;
		else if(iID == 256)
			return ZB_BYLAYER;
	}
	else if(dwKind & ZB_LTYPE){
		//==0ÎªËæ²ã
		if(iID == 0)
			return  ZB_BYLAYER;
	}
...
{
	int iColor;
	DWORD dwJudge = if_by_layer(ZB_COLOR,iID);
	if(dwJudge & ZB_BYLAYER){
		LAYER layer;
		get_layer(hDrawing,pEntityHeader,layer);
		iColor  = layer.Color;	
	}
...
{
/*
	Circle *pNewCircle = new Circle;
	Point ptCenter;
	ptCenter.setdata(pCircle->Point0.x,pCircle->Point0.y);
	pNewCircle->color(entProp.color);
	pNewCircle->layername(entProp.ch_layer);
	pNewCircle->linestyle(entProp.ch_ltype);
	pNewCircle->setdata(ptCenter,pCircle->Radius);
	Entity_ID id;
	pNewCircle->post2db(database,id);
*/
	return TRUE;
...
{
/*
	RArc *pNewArc = new RArc;
	Point ptCenter;
	ptCenter.setdata(pArc->Point0.x,pArc->Point0.y);
	pNewArc->color(entProp.color);
	pNewArc->layername(entProp.ch_layer);
	pNewArc->linestyle(entProp.ch_ltype);
	pNewArc->setdata(ptCenter,pArc->Radius,pArc->StartAngle,pArc->EndAngle);
	Entity_ID id;
	pNewArc->post2db(database,id);
*/
...
{
/*
	Text *pNewTxt = new Text;
	Point ptCenter;
	ptCenter.setdata(pTxt->Point0.x,pTxt->Point0.y);
	pNewTxt->color(entProp.color);
	pNewTxt->layername(entProp.ch_layer);
	pNewTxt->setdata(ptCenter,pTxt->strText);
	Entity_ID id;
	pNewTxt->post2db(database,id);
	pNewTxt->height(pTxt->TextData.Height);
*/
	return TRUE;
...
/*
	for(int i=0;i<database.num();i++){
		LEnts* pEnt = NULL;
		database.getat(pEnt,i);
		if(pEnt->isa(MY_RUNTIME_CLASS(LLine))){
...
/*OBJHANDLE set_line(Line* pLine)
{
	...
	ENTLINE line;
	line.Point0.x = pLine->startpt().x;
	line.Point0.y = pLine->startpt().y;
	line.Point1.x = pLine->endpt().x;
	line.Point1.y = pLine->endpt().y;
