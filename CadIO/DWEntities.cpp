...
/*-------------------------------------------------------------------*
 ...
 |  Output: TRUE if everything is ok                                 |
 *-------------------------------------------------------------------*/
//BOOL dxfDimLinear( HDXF hDxf, double x1, double y1, double x2,
//				  double y2, double x3, double y3, double angle,
//				  LPCTSTR text )
BOOL dxfDimLinear( HDXF hDxf, double x1, double y1, double x2,
				  double y2, double x3, double y3, double angle,
				  LPCTSTR text,
				  LPCTSTR dim_kind,double dimline_angle)//2.21 add
{
	PDXF			pDxf;
	DXF				oldDxf;
	BOOL			result;
	...
	//ÔÚÐ´ÍêÆäËüÊ±£¬ÐèÒª¼ÓÈë±ê×¢µÄÇãÐ±½Ç¶ÈºÍ±ê×¢µÄÖÖÀà£¬
	if(dimline_angle != 0)
		result &= dxfWriteParamDouble(hDxf, 50, dimline_angle);
	result &= dxfWriteParamString(hDxf, 50, dim_kind);
